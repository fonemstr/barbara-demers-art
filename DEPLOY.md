...
2. Vercel will not hand back the key and secret once they are saved as secret, so put the sandbox values in a local, gitignored `.env.lumaprints` (`LUMAPRINTS_API_KEY`, `LUMAPRINTS_API_SECRET`, `LUMAPRINTS_STORE_ID=88382`) and check the setup (read-only): `node --env-file=.env.lumaprints --import tsx scripts/lumaprints-check.ts`. It prints the store IDs and confirms "0.50in Bleed" matches an option.
3. Place one sandbox order the way a paid checkout would: add `--order fox-in-suit` to the same command. It uses that painting's Lumaprints print size and print file from the live site and refuses to run against production. A preview checkout does not work for this test: no Stripe webhook points at preview deployments.
