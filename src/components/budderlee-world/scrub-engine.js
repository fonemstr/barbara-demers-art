/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   LOCAL CHANGES (barbara-demers-art): the upstream engine assumes it owns the
   whole page. Here it sits between the site header and footer, so scroll is
   measured from the container's own top (`origin`), the fixed layers hide once
   the visitor scrolls past the container, the engine's page-level html/body
   rule is dropped, and mountScrollWorld returns a cleanup function for React.
   ========================================================================== */
...
export function mountScrollWorld(container, config) {
  ...
  // Everything the engine attaches to window, so the cleanup can detach it.
  const listeners = [];
  ...
  let origin = 0;   // page Y of the container's top: scroll is measured from here
  let laidOutW = window.innerWidth;   // width the current layout was computed at (see onResize)

  function layout() {
    vh = window.innerHeight;
    laidOutW = window.innerWidth;
    origin = container.getBoundingClientRect().top + (window.scrollY || window.pageYOffset);
  ...
  function read() {
    ...
    // The layers are position:fixed. Once the flight has finished, slide them up
    // with the page so the last scene scrolls away into the footer, then hide
    // them. Above the container (the site header) counts as the start.
    const past = Math.max(0, raw - totalW * vh);
    container.style.setProperty('--sw-past', past + 'px');
    container.classList.toggle('sw-off', past > vh);
    // Hold the last frame while the finished flight slides away.
    const y = Math.min(Math.max(0, raw), totalW * vh);
    ...
    for (let i = 0; i < N; i++) {
      ...
      const c = copies[i];
      c.style.opacity = cop;
      // Upstream replaced the stylesheet's translateY(-50%) here, which dropped
      // the vertical centring on desktop; keep it (mobile anchors to the bottom).
      const drift = reduce ? 0 : (0.5 - pr) * 4;
      c.style.transform = smallMQ.matches ? `translateY(${drift}vh)` : `translateY(calc(-50% + ${drift}vh))`;
  ...
  on(window, 'scroll', () => { if (!ticking) { ticking = true; requestAnimationFrame(read); } }, { passive: true });
  ...
  return function destroy() {
    ...
    container.classList.remove('sw-root', 'sw-off');
...
function injectCSS() {
  ...
  .sw-root.sw-off .sw-sky,.sw-root.sw-off .sw-scrollbar,.sw-root.sw-off .sw-topbar,.sw-root.sw-off .sw-stage,
  .sw-root.sw-off .sw-copylayer,.sw-root.sw-off .sw-route,.sw-root.sw-off .sw-hint{visibility:hidden;}
  .sw-topbar:empty{display:none;}
  .sw-sky,.sw-scrollbar,.sw-stage,.sw-copylayer,.sw-route,.sw-hint{translate:0 calc(-1 * var(--sw-past,0px));}
